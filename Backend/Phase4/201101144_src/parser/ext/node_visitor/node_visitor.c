...
static BOOL _isCTEName( SqlTraverser *traverser, gsp_node *table){
	Map *map = _getTableInfo(traverser, table);
	return (int)map->get(map, (void*)tft_cteName);
}
...
static void _linkColumnReferenceToTable(SqlTraverser *traverser, gsp_base_statement *stmt, gsp_objectname *cr, ESqlClause pLocation){
	...
	if (_locateVariableOrParameter(traverser, stmt, cr) != -1) return;
		...
		if(!_findContext(sqlTraverser, (gsp_node*)pExpr->caseExpression)){
			...
			if(pExpr->caseExpression->whenClauseItemList!=NULL){
				foreach(element, pExpr->caseExpression->whenClauseItemList){
					...
					_insertIntoList( sqlTraverser, when, context->parent);
					if(when->comparison_expr!=NULL){
						_process_expr(sqlTraverser, _insertExprIntoList(sqlTraverser, when->comparison_expr, context->parent, context->clauseType));
					}
					if(when->return_expr!=NULL){
						_process_expr(sqlTraverser, _insertExprIntoList(sqlTraverser,  when->return_expr, context->parent, context->clauseType));
					}
				}
			}
		}
	...
	if(create->tableElementList!=NULL){
		foreach(cell, create->tableElementList){
			...
			if(constraint->columnList!=NULL){
				foreach(cell, constraint->columnList){
					gsp_objectname *column = (gsp_objectname *)gsp_list_celldata(cell);
			...
			if(constraint->referencedColumnList!=NULL){
				foreach(cell, constraint->referencedColumnList){
					gsp_objectname *column = (gsp_objectname *)gsp_list_celldata(cell);
					_insertIntoList( sqlTraverser,  column, parent);
				}
			}
			if(constraint->keyActions!=NULL){
				foreach(cell, constraint->keyActions){
					gsp_keyAction *keyAction = (gsp_keyAction *)gsp_list_celldata(cell);
					_insertIntoList( sqlTraverser, keyAction, parent);
					if(keyAction->keyReference!=NULL){
						_insertIntoList( sqlTraverser, keyAction->keyReference, parent);
					}
				}
			}
		}
...
void _traverseColumnDefinition( SqlTraverser * sqlTraverser, gsp_columnDefinition * columnDefinition, NodeContext * parent ) 
{
...
void _traverseTableElement( SqlTraverser * sqlTraverser, gsp_tableElement * tableElement, NodeContext * parent ) 
{
...
static List* _traverseSQL(SqlTraverser *traverser, gsp_sql_statement *stmt){
	...
	List* nodeList =  traverser->__nodeList;
	List* entries = traverser->__entries;
	Map* contextMap = traverser->__sqlInfoMap;
	Map* tableInfoMap = traverser->__tableInfoMap;
					...
					if(map->containsKey(map, (void*)((int)sft_table))){
						...
						if(size>1){
							...
							foreach(columnElement, joinItem->usingColumns){
								gsp_objectname *column = ((gsp_resultColumn *)gsp_list_celldata(columnElement))->expr->objectOperand;
								if(column!=NULL){
