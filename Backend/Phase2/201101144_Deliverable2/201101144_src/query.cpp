...
bool check_col(VS fcolumns)
{
	...
	REP(i,fcolumns.size())
	{
		...
		{
			fdistinct.PB(temp.substr(1,temp.size()-2));
...
void selectCommand(string query)
{
	...
	REP(i,fcolumns.size())
	{
		if(fcolumns[i][0]=='(' && fcolumns[i][fcolumns[i].size()-1]==')')
		{
			fcolumns[i]=fcolumns[i].substr(1,fcolumns[i].size()-2);
		}
	}
